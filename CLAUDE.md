...
The full tool reference lives in [README.md](README.md); do not duplicate it here. The tools, grouped: screen capture (`screenshot`, `screenshot_region`, `screenshot_window`, `get_pixel_color`, `list_monitors`), mouse (`click`, `double_click`, `scroll`, `move_mouse`, `mouse_drag`), keyboard (`type_text`, `press_key`, `key_combo`, `key_down`, `key_up`, `paste_text`), windows (`list_windows`, `focus_window`, `get_foreground_window`, `get_window_rect`, `window_action`, `set_window_bounds`), clipboard (`get_clipboard`, `set_clipboard`, `get_clipboard_image`, `set_clipboard_image`), text & image matching (`ocr`, `find_image`), and timing/synchronization (`wait`, `wait_for_pixel`, `wait_for_text`).
