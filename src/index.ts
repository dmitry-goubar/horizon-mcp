...
const SCREEN_PARAM_DESC =
  "Monitor index (the `index` field from `list_monitors`) that x,y are relative to: " +
...
const SCREEN_PARAM_DESC_RW =
  "Monitor index (the `index` field from `list_monitors`) to work in. With it, the " +
...
function screenOffsetPs(screen?: number): string {
  if (screen === undefined) return `$ox = 0; $oy = 0`;
  return `Add-Type -AssemblyName System.Windows.Forms
...
async function screenshotRegion(x: number, y: number, width: number, height: number, screen?: number): Promise<string> {
  return ps(`
...
async function getPixelColor(x: number, y: number, screen?: number): Promise<string> {
  return ps(`
${screenOffsetPs(screen)}
Add-Type @"
using System; using System.Runtime.InteropServices;
...
async function runOcr(x?: number, y?: number, width?: number, height?: number, screen?: number): Promise<string> {
...
async function findImage(
  ...
  x?: number, y?: number, width?: number, height?: number, screen?: number
): Promise<string> {
  const hasRegion = x !== undefined && y !== undefined && width !== undefined && height !== undefined;
...
async function waitForPixel(
  ...
  timeoutMs: number, intervalMs: number, tolerance: number, screen?: number
): Promise<string> {
  const target = parseHexColor(hex); // throws on malformed input
  const start = Date.now();
  for (;;) {
    let c = { R: -1, G: -1, B: -1, Hex: "" };
    try { c = JSON.parse(await getPixelColor(x, y, screen)); } catch { /* keep sentinel */ }
    const matched =
  ...
  tools: [
    {
      name: "screenshot",
      description:
        "Capture one or all screens. Returns a PNG image. Omit 'screen' to capture all monitors combined. Otherwise pass a monitor index from `list_monitors` (the `index` field) — indices are OS enumeration order, NOT necessarily primary-first.",
      inputSchema: {
        type: "object",
        properties: {
          screen: {
    ...
    switch (name) {
      ...
      case "screenshot_region": {
        ...
        const data = await screenshotRegion(
          ...
          requireInt(a.width, "width"), requireInt(a.height, "height"), screen
        );
