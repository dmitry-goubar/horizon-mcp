...
async function ps(script: string): Promise<string> {
  ...
  const { stdout, stderr } = await execAsync(
    ...
    { maxBuffer: 50 * 1024 * 1024, windowsHide: true, timeout: PS_TIMEOUT_MS }
  );
  ...
  try {
    switch (name) {
      ...
      case "click": {
        ...
        await mouseClick(x, y, (a.button as "left" | "right") ?? "left", false);
      ...
      case "scroll": {
        ...
        await scroll(x, y, (a.direction as "up" | "down"), amount);
      ...
      case "mouse_drag": {
        ...
        await mouseDrag(x1, y1, x2, y2, (a.button as "left" | "right") ?? "left");
