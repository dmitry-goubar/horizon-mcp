...
export function escapePsSingleQuote(s) {
...
export function escapeSendKeys(s) {
...
for (let c = 65; c <= 90; c++)
    VK[String.fromCharCode(c).toLowerCase()] = c; // a-z
for (let d = 0; d <= 9; d++)
    VK[String(d)] = 0x30 + d; // 0-9
for (let f = 1; f <= 24; f++)
    VK["f" + f] = 0x6f + f; // F1=0x70 .. F24=0x87
...
export function vkOf(name) {
    const v = VK[name.trim().toLowerCase()];
    if (v === undefined)
        throw new Error(`Unknown key in combo: "${name}"`);
    return v;
}
 ...
 */
export function buildKeyComboLines(keys, times, holdMs) {
    if (keys.length === 0)
        throw new Error("key_combo requires at least one key");
    ...
    const ev = (vk, up) => `[Kbd]::keybd_event(${vk}, 0, ${(VK_EXTENDED.has(vk) ? 1 : 0) | (up ? 2 : 0)}, [UIntPtr]::Zero)`;
    const lines = [];
    for (const m of mods)
        lines.push(ev(m, false));
    for (let i = 0; i < repeat; i++) {
        lines.push(ev(main, false));
        if (holdMs > 0)
            lines.push(`Start-Sleep -Milliseconds ${Math.floor(holdMs)}`);
 ...
 */
export function charToStroke(ch) {
    if (ch.length !== 1)
        return null; // surrogate-pair code points etc.
    if (ch >= "a" && ch <= "z")
        return { vk: ch.charCodeAt(0) - 32, shift: false }; // VK = uppercase
    if (ch >= "A" && ch <= "Z")
        return { vk: ch.charCodeAt(0), shift: true };
    if (ch >= "0" && ch <= "9")
        return { vk: ch.charCodeAt(0), shift: false };
    if (ch === " ")
        return { vk: 0x20, shift: false };
    if (ch === "\t")
        return { vk: 0x09, shift: false };
    if (ch === "\n")
        return { vk: 0x0d, shift: false };
    if (ch in SHIFTED_DIGIT)
        return { vk: SHIFTED_DIGIT[ch], shift: true };
    if (ch in OEM_CHARS)
        return { vk: OEM_CHARS[ch][0], shift: OEM_CHARS[ch][1] };
    return null;
}
/**
 ...
 * wrapper in index.ts.
 */
export function buildTypeLines(text, delayMs, holdMs = 0) {
    ...
    const ev = (vk, up) => `[Kbd]::keybd_event(${vk}, [byte]([Kbd]::MapVirtualKey(${vk}, 0)), ${(VK_EXTENDED.has(vk) ? 1 : 0) | (up ? 2 : 0)}, [UIntPtr]::Zero)`;
    const sleep = (ms) => `Start-Sleep -Milliseconds ${Math.floor(ms)}`;
    const SHIFT = 0x10;
    const lines = [];
    for (const ch of text) {
        const s = charToStroke(ch);
        if (!s)
            continue; // not representable on the US layout — skip
        if (s.shift)
            lines.push(ev(SHIFT, false));
        ...
        if (holdMs > 0)
            lines.push(sleep(holdMs));
        lines.push(ev(s.vk, true));
        if (s.shift)
            lines.push(ev(SHIFT, true));
        if (delayMs > 0)
